...
class UserSubscription(models.Model):
    ...
    total_credits = models.IntegerField(default=10)  # Based on plan
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.plan_expiring_date:  # Ensure plan_expiring_date is set if not already
            self.plan_expiring_date = timezone.now() + relativedelta(months=1)  # Set expiry to 1 month from now
